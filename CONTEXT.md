...
The public wallet and verifier are distinct origins with separate browser storage, and the tested journey makes no peer-origin fetch. Both canonical public origins nevertheless still serve the same application artifact containing both role routes and chunks. A local release candidate emits independent wallet, verifier, and showcase artifacts and passes a simulated-origin 200/404 route and entry-chunk matrix, module-graph guards, content scans, source-map checks, and a complete isolated-artifact browser journey. Protected no-alias candidates from commit `9183e7a` pass the equivalent deployed route/chunk matrix and six-header checks. This is candidate evidence, not production cutover proof: the canonical aliases have not moved and the native browser journey has not been rerun against them. All sealed, durable, passkey-bound, standards-aligned, and cross-instance terms below describe the accepted target and may become public product claims only after their release gates pass.
