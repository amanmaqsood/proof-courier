...
The public wallet, verifier, and evidence room now run as three independent production artifacts. Each artifact exposes only its own routes and entry chunk. The canonical production matrix returns 200 for the owning route and 404 for every peer role route and peer entry chunk. The same gate also inspects module graphs and content, forbids source maps, and checks six security headers on every role.

The complete cross-origin consent and proof-delivery journey passed again after the canonical cutover. The wallet and verifier kept separate browser storage, made no peer-origin fetches, rejected an overreaching request with zero disclosure, and left final submission human-only. The machine-readable production receipts remain the source of truth for these claims.
