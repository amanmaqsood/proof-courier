...
The public `/evidence` route turns the release receipts into an inspectable judge surface: native WebMCP lifecycle proof, 22 adversarial scenarios, six browser journeys, bundle isolation, exact source/deployment identifiers, and explicit limitations.

The release gate runs lint, unit and contract tests, 22 named adversarial judge scenarios, release-copy consistency, a production build, wallet/verifier runtime-isolation checks, and six browser journeys, including the scenario lab, primary keyboard path, and automated serious/critical accessibility scanning on every judge-facing page. It writes machine-readable evidence to:
