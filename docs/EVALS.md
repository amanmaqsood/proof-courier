...
| Canonical production role isolation | Manual candidate deployment and promotion plus public route, chunk, and header matrix | three independent production artifacts; owner routes and chunks return 200, peer role routes and chunks return 404, and six security headers pass in `artifacts/release/role-deployment-candidates.json` |
...
The browser suite independently proves that two separate origins can complete the full journey with separate browser storage and no peer-origin fetches during the tested path, that malformed proof remains recoverable rather than creating a false success state, and that every judge-facing route has no serious or critical axe findings. It also proves same-origin wallet-grant persistence across reload, one atomic claim across two real wallet tabs, and metadata-only cross-tab refresh notification. The role-artifact suite proves the same build isolation locally, while the canonical production matrix proves it on the public Vercel origins. The browser suite does not prove verifier replay consumption across reloads, processes, or application instances, or wallet synchronization across devices or browser profiles.
