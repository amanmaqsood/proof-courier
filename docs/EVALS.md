...
| Protected deployment candidates | Manual `vercel deploy --prod --skip-domain` plus authenticated `vercel curl` matrix | three ready candidates from `9183e7a`; role/peer routes, owner/peer chunks, and six security headers in `artifacts/release/role-deployment-candidates.json`; canonical aliases remain unchanged |
